...
export const setupTinySecp256k1 = cacheCall(async () => {
  ...
  function __initializeContext(): void {
  ...
  function isPointCompressed(p: Uint8Array): boolean {
  ...
  function pointCompress(p: Uint8Array, compressed?: boolean): Uint8Array {
    validate.validatePoint(p);
    const outputlen = assumeCompression(compressed, p);
    try {
      PUBLIC_KEY_INPUT.set(p);
  ...
  function xOnlyPointFromScalar(d: Uint8Array): Uint8Array {
    validate.validatePrivate(d);
    try {
      PRIVATE_KEY_INPUT.set(d);
  ...
  function xOnlyPointFromPoint(p: Uint8Array): Uint8Array {
    validate.validatePoint(p);
    try {
      PUBLIC_KEY_INPUT.set(p);
  ...
  function privateSub(d: Uint8Array, tweak: Uint8Array): Uint8Array | null {
    validate.validatePrivate(d);
    validate.validateTweak(tweak);
  ...
  function sign(h: Uint8Array, d: Uint8Array, e?: Uint8Array): Uint8Array {
  ...
  function verify(
    ...
    try {
      ...
      return wasm.verify(Q.length, strict === true ? 1 : 0) === 1
        ? true
        : false;
    } finally {
      HASH_INPUT.fill(0);
...
export type $TinySecp256k1 = Awaited<ReturnType<typeof setupTinySecp256k1>>;
