...
import { crt128 } from './ctr.mjs';
import { ProgressCallback, ScryptFunc, scryptFunc } from './scrypt.mjs';
...
import { pbkdf2 as _pbkdf2 } from '../../_moke-ethers/pbkdf2/index.mjs';
import { Description } from '../properties/index.mjs';
import { randomBytes } from '../random/index.mjs';
import { computeAddress } from '../transactions/index.mjs';
...
async function _getAccount(
  ...
  if (searchPath(data, 'x-ethers/version') === '0.1') {
    ...
    const entropy = await crt128.decrypt(
      mnemonicKey,
      mnemonicIv,
      mnemonicCiphertext,
    );
...
export async function encrypt(
  ...
  if (entropy) {
    ...
    const mnemonicCiphertext = await crt128.encrypt(
      mnemonicKey,
      mnemonicIv,
      entropy,
    );
