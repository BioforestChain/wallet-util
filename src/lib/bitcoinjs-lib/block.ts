...
export class Block {
  ...
  static async calculateMerkleRoot(
    ...
    const rootHash = await fastMerkleRoot(hashes, bcrypto.hash256);

    return forWitness
      ? await bcrypto.hash256(
  ...
  private async __checkMerkleRoot() {
    if (!this.transactions) throw errorMerkleNoTxes;

    const actualMerkleRoot = await Block.calculateMerkleRoot(this.transactions);
    return this.merkleRoot!.compare(actualMerkleRoot) === 0;
  }

  private async __checkWitnessCommit() {
    if (!this.transactions) throw errorMerkleNoTxes;
    if (!this.hasWitnessCommit()) throw errorWitnessNotSegwit;

    const actualWitnessCommit = await Block.calculateMerkleRoot(
      this.transactions,
      true,
    );
