...
const genNetworksCode = (networks) => {
  let code = '';
  for (const [name, data] of Object.entries(networks)) {
    ...
    const {
      messagePrefix = '',
      bech32 = '',
      bip32: { public: bip32_pub = 0, private: bip32_pri = 0 } = {
        public: 0,
        private: 0,
      },
      pubKeyHash = 0,
    ...
    const networkCode = `N(${JSON.stringify([
      messagePrefix,
      bech32,
      [bip32_pub, bip32_pri],
      pubKeyHash,
    ...
    //   console.warn(`unknown keys in ${name}: ${Object.keys(exts)}`);
    //   code += `{
    //     ...${networkCode},
    //     ${JSON.stringify(exts).slice(1, -1)}
    //   }`;
    // } else {
    // }
    code += ',';
...
console.log(genNetworksCode(networks));
